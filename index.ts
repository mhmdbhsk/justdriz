...
program
	.name("justdriz")
	.description("justdriz - full stack framework next.js just.d and drizzle orm")
	.version("1.2.0");

program
	.command("new")
	.description(
		"initialize a new next.js project using recommended settings for justdriz",
	)
	.argument("<name>", "name of project")
	.option("--pnpm", "run with pnpm", false)
	.option("--bun", "run with bun", false)
	.action(async (name, options) => {
		try {
			if (options.pnpm) {
				await spawnCommand(
					`pnpm create next-app ${name} --typescript --eslint --tailwind --app`,
				);
			} else if (options.bun) {
				await spawnCommand(
					`bun create next-app ${name} --typescript --eslint --tailwind --app`,
				);
			} else {
				await spawnCommand(
					`npx create-next-app ${name} --typescript --eslint --tailwind --app`,
				);
			}
		} catch (error) {}
	});
						...
						choices: [
							{
								...
								value: false,
								description:
									"Installs pinned justdriz dependencies. More stable.",
							},
							{
								name: "latest",
								value: true,
...
`,
	...
	.action(async (table, options) => {
		...
		if (authorizationLevel === "admin" && !fs.existsSync("src/app/(admin)")) {
			log.bgRed(
		...
		if (
			authorizationLevel === "private" &&
			!fs.existsSync("src/app/(private)")
		) {
